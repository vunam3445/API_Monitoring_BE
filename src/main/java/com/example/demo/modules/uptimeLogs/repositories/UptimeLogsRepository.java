...
public interface UptimeLogsRepository extends JpaRepository<UptimeLogs, Long>, JpaSpecificationExecutor<UptimeLogs> {
    ...
    @Query("SELECT l.monitor.id, COUNT(l), COUNT(CASE WHEN l.isUp = true THEN 1 END) " +
           "FROM UptimeLogs l WHERE l.monitor.id IN :monitorIds AND l.checkedAt >= :since " +
           "GROUP BY l.monitor.id")
    List<Object[]> getBulkUptimeStats(@Param("monitorIds") List<UUID> monitorIds, @Param("since") LocalDateTime since);

    @Query("SELECT COUNT(l) FROM UptimeLogs l WHERE l.monitor.userId = :userId AND l.checkedAt >= :start AND l.checkedAt < :end")
    long countTotalByUserInRange(@Param("userId") UUID userId, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("SELECT COUNT(l) FROM UptimeLogs l WHERE l.monitor.userId = :userId AND l.isUp = true AND l.checkedAt >= :start AND l.checkedAt < :end")
    long countUpByUserInRange(@Param("userId") UUID userId, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("SELECT AVG(l.responseTimeMs) FROM UptimeLogs l WHERE l.monitor.userId = :userId AND l.checkedAt >= :start AND l.checkedAt < :end AND l.responseTimeMs IS NOT NULL")
    Double getAvgLatencyByUserInRange(@Param("userId") UUID userId, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
    ...
    @Query(value = "SELECT COUNT(l.id) AS total, COUNT(CASE WHEN l.is_up = false THEN 1 END) AS failed " +
            "FROM uptime_logs l WHERE l.checked_at >= :since", nativeQuery = true)
    List<Object[]> getGlobalErrorRateStats(@Param("since") LocalDateTime since);

    @Query("SELECT l FROM UptimeLogs l ORDER BY l.checkedAt DESC")
    Page<UptimeLogs> findLatestLogsGlobal(Pageable pageable);

    @Query(value = "SELECT " +
            "to_timestamp(floor(extract(epoch from l.checked_at) / :seconds) * :seconds) AS time, " +
            "AVG(l.response_time_ms) AS avgLatencyMs " +
            "FROM uptime_logs l " +
    ...
    @Query(value = "SELECT COUNT(l.id) AS total, COUNT(CASE WHEN l.is_up = true THEN 1 END) AS successful " +
            "FROM uptime_logs l WHERE l.checked_at >= :since", nativeQuery = true)
    List<Object[]> getGlobalUptimeStats(@Param("since") LocalDateTime since);

    @Query(value = "SELECT m.method AS method, COUNT(l.id) AS count " +
            "FROM uptime_logs l " +
            "JOIN monitors m ON l.monitor_id = m.id " +
            "WHERE l.checked_at >= :since " +
            "GROUP BY m.method", nativeQuery = true)
